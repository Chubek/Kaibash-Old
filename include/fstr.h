...
extern fstr_s *fstr_new(char const *filename);
extern fstr_s *fstr_new_from_buffer(char const *buffer);

fstr_s *fstr_copy(fstr_s const *in, size_t start, size_t len);
extern void fstr_show(fstr_s const *fstr);
extern void fstr_free(fstr_s *in);
...
extern fstr_list fstr_split (fstr_s const *in, gchar const *start_pattern);
extern void fstr_list_free(fstr_list in);